...
export default function ClientLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <AuthProvider>
      <div className="flex h-screen m-0 bg-slate-50 font-sans antialiased overflow-hidden selection:bg-riverside-red/10 selection:text-riverside-red">
        {/* Bouton Menu Hamburger Flottant (Visible uniquement sur mobile) */}
        {pathname !== '/login' && (
          <button 
        ...
        {/* Menu latéral fixe avec état retractable (Desktop) */}
        {pathname !== '/login' && (
          <aside 
            className={cn(
        ...
        <main className="flex-1 h-screen overflow-hidden bg-slate-50 relative flex flex-col w-full">
          <SonnerToaster position="top-right" expand={true} richColors />
          <HotToaster position="top-center" />
          <div className="flex-1 overflow-y-auto overflow-x-hidden scroll-smooth scrollbar-hide w-full">
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className={cn(
                  "max-w-[1600px] mx-auto min-h-screen w-full transition-all duration-300",
                  pathname === '/login' ? "p-0" : "p-4 sm:p-6 md:p-10"
                )}
              >
                {children}
              </motion.div>
